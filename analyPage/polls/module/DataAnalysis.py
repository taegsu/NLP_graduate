...
class MentionAnalysis():
    ...
    def get_year_list(self):
        ...
        for year in range(now.tm_year,now.tm_year-3,-1): ##현재 연도로부터 3년간 데이터 존재하는지 
            df = pandas.read_csv("polls/module/Crawler/rawdata/contents_musinsa.csv",encoding='utf-8',index_col=False)
    ...
    def set_Media(self,media):
        ...
        if self.media == self.MEIDA_INSTA:
            self.filepath='polls/module/Crawler/rawdata/contents_instagram.csv'
            self.date_pattern = "%s-%s-"
        elif self.media == self.MEDIA_MUSINSA:
            self.filepath='polls/module/Crawler/rawdata/contents_musinsa.csv'
            self.date_pattern = "%s.%s."
        else :
            self.media = None
